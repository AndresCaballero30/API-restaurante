...
app.get('/api/productos', (req, res) => {
    try {
        ...
        const productos = runQuery('SELECT * FROM Productos WHERE is_active = 1');
        res.json(productos);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.get('/api/productos/:id', (req, res) => {
    try {
        const { id } = req.params;
        // Only fetch active products
        const producto = runQuery('SELECT * FROM Productos WHERE id_producto = ? AND is_active = 1', [id]);
...
app.delete('/api/productos/:id', (req, res) => {
    try {
        ...
        const result = runQuery(
            'UPDATE Productos SET is_active = 0 WHERE id_producto = ?',
            [id]
        );
        if (result.changes > 0) {
            res.json({ message: 'Producto desactivado (borrado lógico)' });
        } else {
            res.status(404).json({ message: 'Producto no encontrado' });
        }
    } catch (error) {
...
app.post('/api/pedidos', (req, res) => {
    const { id_cliente, id_empleado, id_mesa, fecha, total, estado, currentGuests } = req.body;

    if (!id_mesa) {
        return res.status(400).json({ message: 'El id_mesa es requerido.' });
    }

    const createOrderAndUpdateTable = db.transaction(() => {
        ...
        // 2. Update the table's status and guest count
        runQuery(
            'UPDATE Mesas SET estado = ?, currentGuests = ? WHERE id_mesa = ?',
            ['Ocupada', currentGuests, id_mesa]
        );
    ...
    try {
        const result = createOrderAndUpdateTable();
        res.status(201).json({ id_pedido: result.lastInsertRowid });
    } catch (error) {
        console.error('Transaction error in POST /api/pedidos:', error);
        res.status(500).json({ error: 'Database operation failed during order creation' });
    }
});
...
const addOrderDetail = db.transaction((orderDetail) => {
    const { id_producto, cantidad } = orderDetail;

    // 1. Verificar stock
    const producto = db.prepare('SELECT nombre, stock FROM Productos WHERE id_producto = ?').get(id_producto);
    ...
    if (producto.stock < cantidad) {
        ...
        err.productName = producto.nombre;
        throw err;
    }

    // 2. Actualizar el stock
    db.prepare('UPDATE Productos SET stock = stock - ? WHERE id_producto = ?').run(cantidad, id_producto);

    // 3. Insertar el detalle del pedido
    const result = db.prepare(
        'INSERT INTO Detalles_Pedidos (id_pedido, id_producto, cantidad, precio_unitario) VALUES (?, ?, ?, ?)'
    ).run(orderDetail.id_pedido, id_producto, cantidad, orderDetail.precio_unitario);
...
app.post('/api/detalles-pedidos', (req, res) => {
    try {
        const id_detalle = addOrderDetail(req.body);
        res.status(201).json({ id_detalle });
    } catch (error) {
        if (error.message === 'PRODUCT_NOT_FOUND') {
            return res.status(404).json({ message: 'El producto no fue encontrado.' });
        }
        if (error.message === 'INSUFFICIENT_STOCK') {
            return res.status(409).json({ message: `Stock insuficiente para el producto: ${error.productName}` });
        }
        console.error('Error in /api/detalles-pedidos:', error);
